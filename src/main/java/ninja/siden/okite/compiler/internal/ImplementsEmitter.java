 ...
 * @author taichi
 */
public class ImplementsEmitter implements ConstraintEmitter {

	final String impl;

	public ImplementsEmitter(String className) {
		this.impl = className;
	}
	...
	public void emit(ProcessingEnvironment env, PrintWriter pw,
		...
		emit(env, pw, am, field, field.asType());
	}

	void emit(ProcessingEnvironment env, PrintWriter pw, AnnotationMirror am,
			Element element, TypeMirror type) {
		String t = new Elements(env).toBoxedClassName(type);
		pw.printf("%s<%s> c = new %s<>();", this.impl, t, this.impl);
	...
	public void emit(ProcessingEnvironment env, PrintWriter pw,
		...
		emit(env, pw, am, method, method.getReturnType());
	}
}

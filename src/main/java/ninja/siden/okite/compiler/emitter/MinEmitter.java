 ...
 * @author taichi
 */
public class MinEmitter extends DefaultEmitter {
		...
		return constType.map(
			...
			// TODO collect error message
				env.env.getMessager().printMessage(
						Kind.ERROR,
						"@Min can apply to Number(including primitives) and CharSequence. "
	...
	protected Optional<String> decide(Env env, TypeMirror target) {
		...
		if (typeUtils
			...
			return Optional
					.of(MinConstraint.ForDouble.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target, elemUtils.boxedType(TypeKind.FLOAT))) {
			return Optional.of(MinConstraint.ForFloat.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(BigDecimal.class).get().asType())) {
			return Optional.of(MinConstraint.ForBigDecimal.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(BigInteger.class).get().asType())) {
			return Optional.of(MinConstraint.ForBigInteger.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(Number.class).get().asType())) {
			return Optional
					.of(MinConstraint.ForNumber.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(CharSequence.class).get().asType())) {
			return Optional.of(MinConstraint.ForCharSequence.class
					.getCanonicalName());
		}

 ...
 * @author taichi
 */
public class MaxEmitter extends DefaultEmitter {
		...
		return constType.map(
			...
			// TODO collect error message
				env.env.getMessager().printMessage(
						Kind.ERROR,
						"@Max can apply to Number(including primitives) and CharSequence. "
	...
	protected Optional<String> decide(Env env, TypeMirror target) {
		...
		if (typeUtils
			...
			return Optional
					.of(MaxConstraint.ForDouble.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target, elemUtils.boxedType(TypeKind.FLOAT))) {
			return Optional.of(MaxConstraint.ForFloat.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(BigDecimal.class).get().asType())) {
			return Optional.of(MaxConstraint.ForBigDecimal.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(BigInteger.class).get().asType())) {
			return Optional.of(MaxConstraint.ForBigInteger.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(Number.class).get().asType())) {
			return Optional
					.of(MaxConstraint.ForNumber.class.getCanonicalName());
		}
		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(CharSequence.class).get().asType())) {
			return Optional.of(MaxConstraint.ForCharSequence.class
					.getCanonicalName());
		}

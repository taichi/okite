...
import ninja.siden.okite.constraint.NotEmptyConstraint;

/**
 * @author taichi
 */
public class NotEmptyEmitter extends DefaultEmitter {
		...
		return constType.map(
			...
			// TODO collect error message
				env.env.getMessager().printMessage(
						Kind.ERROR,
						"@NotEmpy can apply to Optional, CharSequence, Collection, Map, Array. "
	...
	protected Optional<String> decide(Env env, TypeMirror target) {
		...
		if (target.getKind() == TypeKind.ARRAY) {
			return Optional.of(NotEmptyConstraint.ForArray.class
					.getCanonicalName());
		}

		if (typeUtils.isAssignable(target,
				elemUtils.getTypeElement(CharSequence.class).get().asType())) {
			return Optional.of(NotEmptyConstraint.ForCharSequence.class
					.getCanonicalName());
		}
		...
		if (typeUtils.isAssignable(erasure,
			...
			return Optional.of(NotEmptyConstraint.ForOptional.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(erasure,
				elemUtils.getTypeElement(Collection.class).get().asType())) {
			return Optional.of(NotEmptyConstraint.ForCollection.class
					.getCanonicalName());
		}
		if (typeUtils.isAssignable(erasure, elemUtils.getTypeElement(Map.class)
				.get().asType())) {
			return Optional.of(NotEmptyConstraint.ForMap.class
					.getCanonicalName());
		}
